# -*- coding: utf-8 -*-
"""This class contains helper functions to create a PSF using either 
PSFr or STARRED."""
...
class PSF:
    """This class contains helper functions to create a PSF estimate
    ...
    def __init__(self, io_directory, lens_name, data_band, instrument):
        ...
        self.image_file_name = f"{self.full_image_path}/full_image_{self.lens_name}_{self.data_band}.fits"
        self.weight_file_name = f"{self.full_image_path}/weight_image_{self.lens_name}_{self.data_band}.fits"

    def get_kwargs_cut(self):
        """
        Estimate the cuts in the different Source Extractor quantities for masking, and get the 
        ...
        """
        catalog = preprocessing_util.make_image_catalog(self.io_directory, self.lens_name, self.data_band)
    ...
    def get_psf_candidates(
        ...
        """
        Obtain PSF candidates based upon the catalog and cutting dictionary. In addition to
        cutouts of the candidate objects being created, weight cutouts and noise map cutouts
        are made. To save the cutouts, toggle `save = True`. Can be run again excluding 
        specific objects to narrow down the initial candidates. 
        ...
        if exclude_specific is not None:
            ...
            print(
                f"Excluded {len(exclude_specific)} specified objects."
            )
        ...
        mean_bkd, sigma_bkd = preprocessing_util.get_background(self.io_directory, self.lens_name, self.data_band)
        if self.instrument == "HST":
            ...
            for idx in candidate_idx:
                ...
                noise_maps.append(
                    np.abs(exposure) / weight + sigma_bkd**2
                )
            ...
            for x, y in zip(catalog.data["X_IMAGE"][mask], catalog.data["Y_IMAGE"][mask]):
                exposure = Cutout2D(image_reduced, (x, y), size=cutout_size).data
                # remove objects with NaN pixels
                if not np.all(np.isfinite(exposure)):
                    mask[idx] = False
                ...
                star_weights.append(weight)
                noise_maps.append(
                    Cutout2D(err, (x, y), size=cutout_size).data
                )
        else:
        ...
        self.plot_psf_candidates(mask=mask,
        ...
        if save:
            self.file_system.save_star_cutouts(psf_class=self,
                                               star_exposures=star_exposures, 
                                               star_weights=star_weights, 
                                               noise_maps=noise_maps
                                            )
    ...
    def make_candidate_mask(self, star_num, kwargs_mask, save=False):
        """Create a mask for a PSF candidate object and save it in the
        expected :meth:`~dolphin.preprocessing.psf.PSF` format.
        ...
        :type save: `bool`
        """
        star_data_list, _, weight_maps, noise_maps = self.load_psf_candidate_attributes()
        ...
        plt.tight_layout()
        if save:
            self.file_system.save_psf_candidate_mask(self.lens_name, self.data_band, star_num, mask)

    def make_psf_psfr(self, 
                      oversampling=1, 
                      error_map_list=None,
        ...
        psf_returns = psfr.stack_psf(star_list=star_data_list, 
        ...
        error_map = psfr.psf_error_map(star_list=star_data_list, 
        ...
        if oversampling > 1:
            ...
            error_map_mask = (
                final_psf_mask
                .reshape(
                    native_nx, oversampling,
                    native_ny, oversampling
                )
                .any(axis=(1, 3))
            )
        
            final_error_map = np.where(
                error_map_mask,
                error_map,
                0
            )
        ...
        self.plot_psf_and_variance_map(method="PSFr",
                                        psf_guess=psf_guess,
        ...
        if save:
            self.file_system.save_psf_and_variance_map(lens_name=self.lens_name,
    ...
    def make_psf_starred(self, 
        ...
        star_data_list, mask_data_list, _, noise_maps = self.load_psf_candidate_attributes()
        variance = noise_maps**2

        model = STARRED_PSF(image_size=star_data_list[0].shape[1],
        ...
        kwargs_init, kwargs_fixed, kwargs_up, kwargs_down = model.smart_guess(data=star_data_list,
                                                                              fixed_background=True
                                                                            )

        parameters = ParametersPSF(kwargs_init=kwargs_init,
                                   kwargs_fixed=kwargs_fixed,
                                   kwargs_up=kwargs_up,
                                   kwargs_down=kwargs_down
                                )

        loss = Loss(data=star_data_list,
                    psf_class=model,
        ...
        optim = Optimizer(loss_class=loss,
                          param_class=parameters,
                          method="Newton-CG"
                        )
        ...
        parameters = ParametersPSF(kwargs_init=kwargs_partial,
                                   kwargs_fixed=kwargs_fixed,
                                   kwargs_up=kwargs_up,
                                   kwargs_down=kwargs_down
                                )
        ...
        error_map = model.get_psf_error_map(kwargs=kwargs_final,
                                            data=star_data_list,
                                            sigma_2=variance,
                                            masks=mask_data_list,
        ...
        self.plot_psf_and_variance_map(method="STARRED",
                                         psf_guess=psf_guess,
                                         variance_map=error_map**2,
                                         psf_cut=final_psf,
                                         variance_map_cut=final_error_map**2,
    ...
    def plot_psf_candidates(self,
        ...
        star_coords_list = [
            (int(i), int(j)) for i, j in zip(catalog.data[mask]["X_IMAGE"], catalog.data[mask]["Y_IMAGE"])
        ]
        ...
        im = ax.matshow(plt_data, origin="lower", vmin=-1, vmax=2.)
        ax.autoscale(False)
    ...
    def plot_saved_psf_candidates(self):
        """Plot the saved star cutouts, weight cutouts, and noise map cutouts with
        their correspdoning masks to probe appropriate mask configurations.
        ...
        star_exposures, mask_data_list, star_weights, noise_maps = self.load_psf_candidate_attributes()
    ...
    @staticmethod
    def plot_psf_and_variance_map(method,
                                psf_guess, 
                                variance_map,
                                psf_cut=None, 
        ...
        if method == "PSFr":
            ...
            if psf_cut is not None and variance_map_cut is not None:
                ...
                cut_fraction = 100 * (
                    1 - np.sum(psf_cut) / np.sum(psf_guess)
                )
                ax[0].set_xlabel(
                    f"Cut Fraction: {abs(cut_fraction):.2f}%"
                )
    ...
    def load_psf_candidate_attributes(self):
        ...
        star_data_list, mask_data_list, weight_maps, noise_maps = self.file_system.load_psf_candidate_attributes(self.lens_name, self.data_band)
