...
class TestPSF(object):
    ...
    def mock_catalog(self):
        return fits.BinTableHDU.from_columns([
            ...
            fits.Column(
                name="ELLIPTICITY",
                format="E",
                array=np.array([0.05, 0.1, 0.3])
            ),
            fits.Column(
                name="CLASS_STAR",
                format="E",
                array=np.array([0.9, 0.8, 0.2])
            ),
            fits.Column(
                name="X_IMAGE",
                format="E",
                array=np.array([500., 600., 700.])
            ),
            fits.Column(
                name="Y_IMAGE",
                format="E",
    ...
    @patch("dolphin.preprocessing.preprocessing_util.make_image_catalog")
    def test_get_kwargs_cut(self, mock_make_catalog, mock_catalog):
        mock_make_catalog.return_value = mock_catalog
    
        kwargs_cut, catalog = self.psf.get_kwargs_cut()
    
        assert catalog is mock_catalog
        assert "MagMaxThresh" in kwargs_cut
        assert "MagMinThresh" in kwargs_cut
        assert "SizeMinThresh" in kwargs_cut
        assert "SizeMaxThresh" in kwargs_cut
        assert kwargs_cut["EllipticityThresh"] == 0.1
        assert kwargs_cut["ClassStarMax"] == 1.0
        assert kwargs_cut["ClassStarMin"] == 0.5
    
        mock_make_catalog.assert_called_once_with(
            self.psf.io_directory,
            self.psf.lens_name,
            self.psf.data_band,
        )
    ...
    def test_get_psf_candidates(
        ...
        # Cutouts should reflect Cutout2D behavior
        assert stars[0].shape == (100, 100)
        assert weights[0].shape == (100, 100)
        assert noise[0].shape == (100, 100)
    
        # Since inputs are all ones and sigma_bkd = 1:
        # noise = abs(exp)/wht + sigma^2 = 1 + 1 = 2
        np.testing.assert_allclose(stars[0], 1.0)
        np.testing.assert_allclose(weights[0], 1.0)
        np.testing.assert_allclose(noise[0], 2.0)
    ...
    def test_make_psf_psfr(
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                final_psf, variance_map = (
                    self.psf.make_psf_psfr(
                        cut_threshold=1e-20,
                        save=False,
                    )
                )
    ...
    def test_plot_psf_candidates(
        ...
        mock_fits.return_value.__enter__.return_value = [
            fake_hdu
        ]
        ...
        wcs.all_pix2world.side_effect = (
            lambda x, y, z: (x, y)
        )
    
        wcs.all_world2pix.side_effect = (
            lambda x, y, z: (x, y)
        )
    ...
    def test_plot_psf_and_variance_map_psfr(
        ...
        """Test that the PSFr branch of `plot_psf_and_variance_map` operates as expected."""
        psf = np.ones((5, 5))
    ...
    def test_plot_psf_and_variance_map_starred(
        ...
        """Test that the STARRED branch of `plot_psf_and_variance_map` operates as expected."""
        psf = np.ones((5, 5))
        variance = np.ones((5, 5)) * 0.1
        ...
        mock_plot_loss.assert_called_once_with(
            [10, 5, 1]
        )
    ...
    def test_plot_psf_and_variance_map_psfr_no_cut(
        ...
        """Test that `plot_psf_and_variance_map` creates only one plot
        if there is no variance map."""
        psf = np.ones((5, 5))
    ...
    def test_load_psf_candidate_attributes(self):
        """Test that `load_psf_candidate_attributes` returns
        expected components."""
        expected = (
            ["stars"],
            ["masks"],
