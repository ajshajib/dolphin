...
class TestPSF(object):
    ...
    def test_make_image_catalog(
        ...
        """Test that `make_image_catalog` runs and creates the properly
        named file."""
        catalog = MagicMock()
        ...
        mock_fits.assert_called_once_with(
            "MOCK_F814W.cat"
        )
    ...
    def test_get_kwargs_cut(self):
        ...
        with patch.object(
            ...
            kwargs_cut, returned_catalog = (
                self.psf.get_kwargs_cut()
            )
    ...
    def test_get_psf_candidates(
        ...
        """Test that `get_psf_candidates` returns the expected number
        of cutouts, weights, and noise maps."""
        kwargs_cut = {
            "SizeMaxThresh": 5,
            "SizeMinThresh": 1,
        ...
        mock_system_instance.get_full_image.return_value = np.ones(
            (1000, 1000)
        )
    
        mock_system_instance.get_full_exposure.return_value = np.ones(
            (1000, 1000)
        )
