...
class PSF:
    ...
    def get_kwargs_cut(self):
        """
        Estimate the cuts in the different SExtractor quantities for masking, and get the 
        SExtractor catalog.
    ...
    def get_psf_candidates(
        ...
        if self.instrument == "HST":
            ...
            system.add_image_data_init(self.data_band, 
            ...
            star_exposures = analysis.get_objects_image(img_full - mean_bkd, catalog, mask, 50) # get star cutouts, background subtracted
            
            wht_full = system.get_full_exposure(self.data_band) # get the full exposure map
            wht_full[wht_full <= 0] = 10**(-10)
            star_weights = analysis.get_objects_image(wht_full, catalog, mask, 50) # get weight cutouts for stars
    ...
    def make_psf_psfr(self, 
        ...
        if save:
            self.file_system.save_psf_and_variance_map(psf_class=self,
                                                    psf_guess=final_psf,
    ...
    def load_psf_candidate_attributes(self):
        ...
        star_data_list, mask_data_list, weight_maps, noise_maps = self.file_system.load_psf_candidate_attributes(self)
